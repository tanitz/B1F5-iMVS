...
def FIND_POINT1(CropImage):
    ...
    EDParams.MinPathLength = 250    # try changing this value between 5 to 1000
    ...
    if lines is not None: 
        ...
        # for i in min(np.int16(lines[i][0][0])):
        #     Dline.append(lines[i])
        # for i in max(np.int16(lines[i][0][0])):
        #     Dline.append(lines[i])    
        print("lines :",len(lines))
...
def grab_img():
    ...
    while True:
        ...
        try :
            ROI_Crop2 = [500,900,700,1300] 
            ROI_Crop1 = [1250,1450,700,1300]
            ...
            grayL = cv.bilateralFilter(grayL, 13,5,15)
            # grayL = cv.GaussianBlur(grayL, (3,3), 0)
            edges = cv.Canny(grayL, 85, 110)
            contoursFrist, _ = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
            contours = []
            cv.drawContours(crop1, contoursFrist, -1, (255, 255, 0), 2)  
            # cv.imshow('crop11', crop1)
    ...
    # **********************************************************************************************
            ...
            crop2 = cv.bilateralFilter(crop2, 13,15,15)
    ...
    # **********************************************************************************************
        ...
        try :
            ...
            # P3
            Distance_P3 = abs(P1y_origin1-P2y_origin1)*Pixel_size2
            # P2
            Distance_P2 = abs(P2x_origin1-P2x_origin2)
            # P4
            Distance_P4 = Dcircle[0]*Pixel_size
            # P5
        ...
        try :
            ...
            cv.arrowedLine(Image_output, (Detectedline[0][0][0]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), (Detectedline[1][0][2]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), color_arrow, 2, cv.LINE_AA)
            cv.arrowedLine(Image_output, (Detectedline[1][0][2]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), (Detectedline[0][0][0]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), color_arrow, 2, cv.LINE_AA)
            ...
            cv.putText(Image_output,"P1 = "+f"{Distance_P1:.2f}" + "  mm." ,(30, 50+80*1), cv.FONT_HERSHEY_SIMPLEX ,0.9,color_text, 3, cv.LINE_AA)
            cv.putText(Image_output,"P2 = "+f"{Distance_P2:.2f}" + "  mm." ,(30, 50+80*2), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
            cv.putText(Image_output,"P3 = "+f"{Distance_P3:.0f}" + "  mm." ,(30, 50+80*3), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
            cv.putText(Image_output,"P4 = "+f"{Distance_P4:.2f}" + "  mm." ,(30, 50+80*4), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
            cv.putText(Image_output,"P5 = "+f"{Distance_P5:.2f}" + "  mm." ,(30, 50+80*5), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
            cv.putText(Image_output,"OK" ,(30, 50+80*6), cv.FONT_HERSHEY_SIMPLEX ,0.9, (0,255,0),5, cv.LINE_AA)
...
server_socket1.connect(('127.0.0.1',5000))
