...
def FIND_POINT2(CropImage):
    ...
    if ellipses is not None: # Check if circles and ellipses have been found and only then iterate over these and add them to the image
        for i in range(len(ellipses)):
            ...
            major_axis = axes[0]
            minor_axis = axes[1]
            if abs(major_axis-minor_axis) < 3 :
...
def grab_img():
    ...
    while True:
        ...
        # ROI_Crop2 = [300,800,2500,3200] 
        # ROI_Crop1 = [1000,1500,2500,3200]
        # ROI_Crop3 = [1600,2200,2500,3200]

        ROI_Crop2 = [400,800,700,1300] 
        ROI_Crop1 = [1100,1250,700,1300]
        ROI_Crop3 = [1500,2000,700,1300]
        ...
        grayL = cv.GaussianBlur(grayL, (5, 5), 0)
        edges = cv.Canny(grayL, 10, 150)
        contoursFrist, _ = cv.findContours(edges, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)
        # print("contoursFrist :",len(contoursFrist))
        contours = []

        for contour in contoursFrist:
        ...
        cv.drawContours(crop1, contours, -1, (0, 255, 0), 2)  
        # print("con = "+str(len(contours)))
        cv.imshow('crop1', crop1)
        # cv.waitKey(0)
        min_x_point1 = min(contours[1][:, 0, 0])
        min_x_index1 = np.where(contours[1][:, :, 0] == min_x_point1)[0][0]
        min_y_point1 = contours[1][min_x_index1][0][1]
        max_x_point2 = max(contours[0][:, 0, 0])
        max_x_index2 = np.where(contours[0][:, :, 0] == max_x_point2)[0][0]
        max_y_point2 = contours[0][max_x_index2][0][1]
        avg_y_point = (max_y_point2+min_y_point1)//2
        convex_Point1 = (min_x_point1+ROI_Crop1[2]//K,(avg_y_point)+ROI_Crop1[0]//K)
        convex_Point2 = (max_x_point2+ROI_Crop1[2]//K,(avg_y_point)+ROI_Crop1[0]//K)
        # print(str(convex_Point1),str(convex_Point2))
        P2x_origin1,P2y_origin1 =convex_Point1
        P2x_origin2,P2y_origin2 =convex_Point2
        cv.circle(crop1, (min_x_point1,avg_y_point), 3, (0,0,255), -1)   
        cv.circle(crop1, (max_x_point2,avg_y_point), 3, (0,0,255), -1)
...
# **********************************************************************************************
        ...
        crop2 = cv.GaussianBlur(crop2, (13,13), 0, 0.7)
        Detectedline,min_pointP3 =  FIND_POINT1(crop2)
...
# **********************************************************************************************
        ...
        # cv.circle(Image_output, (P2x_origin1,P2y_origin1), 50, (0,255,0), -1)   
        # cv.circle(Image_output, (P2x_origin2,P2y_origin2), 50, (0,255,0), -1) 
        # try :
        ...
        # cv.circle(Image_output, (P1x_origin1,(P1y_origin1+P1y_origin2)//2), 10, (255,0,0), -1) 
        # cv.circle(Image_output, (P1x_origin2,(P1y_origin1+P1y_origin2)//2), 10, (255,0,0), -1) 
        ...
        try :
        # P1
            Distance_P1 = abs(P1x_origin2-P1x_origin1)
            # P3
            Distance_P3 = abs(P1y_origin1-P2y_origin1)
            # P2
            Distance_P2 = abs(P2x_origin1-P2x_origin2)
            # P4
            Distance_P4 = Dcircle[0]
            # P5
            Distance_P5 = Dcircle[1]
        except :
            continue
        ...
        cv.arrowedLine(Image_output, (Detectedline[0][0][0]+ ROI_Crop2[2]//K, (Detectedline[0][0][1]//2)+ ROI_Crop2[0]//K), (Detectedline[1][0][2]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), color_arrow, 2, cv.LINE_AA)
        cv.arrowedLine(Image_output, (Detectedline[1][0][2]+ ROI_Crop2[2]//K, (Detectedline[1][0][3]//2)+ ROI_Crop2[0]//K), (Detectedline[0][0][0]+ ROI_Crop2[2]//K, (Detectedline[0][0][1]//2)+ ROI_Crop2[0]//K), color_arrow, 2, cv.LINE_AA)
        ...
        P4_start = (centerX_circle - Dcircle[1] // 2+ROI_Crop3[2]//2, centerY_circle+ROI_Crop3[0]//2 )
        P4_end = (centerX_circle + Dcircle[1] // 2+ROI_Crop3[2]//2, centerY_circle+ROI_Crop3[0]//2 )
        ...
        # start_point_v = (centerX_circle+ROI_Crop3[2]//2, centerY_circle - Dcircle[1]//2+ROI_Crop3[0]//2)
        # end_point_v = (centerX_circle+ROI_Crop3[2]//2, centerY_circle + Dcircle[1]//2+ROI_Crop3[0]//2)
        # print(Position_P4)
        cv.arrowedLine(Image_output, P4_start, P4_end, color_arrow, 2)
        cv.arrowedLine(Image_output, P4_end,P4_start, color_arrow, 2)
        cv.putText(Image_output,"P5", Position_P4, cv.FONT_HERSHEY_SIMPLEX ,1.5, color_text, 2, cv.LINE_AA)
        ...
        cv.putText(Image_output,"P1 = "+str(Distance_P1) + "  Pixel" ,(30, 50+80*1), cv.FONT_HERSHEY_SIMPLEX ,0.9,color_text, 3, cv.LINE_AA)
        cv.putText(Image_output,"P2 = "+str(Distance_P2) + "  Pixel" ,(30, 50+80*2), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
        cv.putText(Image_output,"P3 = "+str(Distance_P3) + "  Pixel" ,(30, 50+80*3), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
        cv.putText(Image_output,"P4 = "+str(Distance_P4) + "  Pixel" ,(30, 50+80*4), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
        cv.putText(Image_output,"P5 = "+str(Distance_P5) + "  Pixel" ,(30, 50+80*5), cv.FONT_HERSHEY_SIMPLEX ,0.9, color_text,3, cv.LINE_AA)
...
# **********************************************************************************************
        ...
        # cv.imshow('detected circles and result',cv.resize(Image_output,(684,456)))
